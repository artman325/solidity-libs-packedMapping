...
describe("Staking contract tests", function () {
    ...
    it("test", async() => {
        ...
        let inputArr = [14,2,3,5,89,8,24,4,85,6,7,8,23,2342,2344,224,234,4];
        
        for (let i = 0; i< inputArr.length; i++) {
            await MockContractInstance.set(BigNumber.from(i), BigNumber.from(inputArr[i]));
        }
        
    }); 

    it("test 1k iterations with lib packing", async() => {
        
        for (let i = 0; i< 1000; i++) {
            await MockContractInstance.set1000Lib(BigNumber.from(i), BigNumber.from(i));
        }
        
    }); 

    it("test 1k iterations with contract mapping", async() => {
        
        for (let i = 0; i< 1000; i++) {
            await MockContractInstance.set1000Map(BigNumber.from(i), BigNumber.from(i));
        }
